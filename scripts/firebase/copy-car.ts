 ...
 */
async function copyDocument(sourceCarId: string, destCarId: string): Promise<void> {
  try {
    ...
    const damageEntriesSnapshot = await _db
      .collection("cars")
      .doc(sourceCarId)
      .collection("damages")
      .get()
    ...
    if (!damageEntriesSnapshot.empty) {
      ...
      await batch.commit()
      console.log(
        `✅ Successfully copied ${damageEntriesSnapshot.size} damage entries from ${sourceCarId} to ${destCarId}`
      )
    } else {
