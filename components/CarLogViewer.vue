...
const { logs, isLoading, error } = useCarLogs(props.carId)
  ...
  <div>
    ...
    <template v-else>
      <div class="space-y-2">
        ...
        >
          <div v-if="user" class="font-semibold text-sm mb-1">{{ log.userName }}</div>
          ...
          <div v-if="user && log.notes" class="mt-1 text-sm text-gray-700 italic">
            {{ log.notes }}
          </div>
        </div>
      </div>
